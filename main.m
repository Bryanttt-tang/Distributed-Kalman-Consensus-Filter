...
[A_dis,B_dis,X1] = dynamics(A,B,C,dt,x_ref1,time,vu,L);
[A_dis,B_dis,X2] = dynamics(A,B,C,dt,x_ref2,time,vu,L);
[A_dis,B_dis,X3] = dynamics(A,B,C,dt,x_ref3,time,vu,L);
[A_dis,B_dis,X4] = dynamics(A,B,C,dt,x_ref4,time,vu,L); %A_dis are assumed to be the same for all quadrators
...
plot_R(t_vec,X1,X_hat21,X_hat31,X_hat41,X_hat_sta,k,3,gamma); % comparison when R is/not constant
%plot_DKCF(t_vec,X1,X_hat_sta,X_plus_sta,X_min_sta,n_sen);
%plot_variance(t_vec,P_M21,P_M31,P_M41,P_M_1,P_M_sta,k,eps,gamma)